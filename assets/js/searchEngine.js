...
async function loadCommunityData() {
  ...
  if (!supabase) {
    console.warn('Supabase not available, waiting...');
    let retries = 0;
  ...
  if (!supabase) {
    console.error('❌ Supabase still not available after waiting');
    isLoading = false;
    return;
  }
...
async function loadSkillsList() {
  ...
  try {
    ...
    console.log(`📚 Loaded ${allSkills.length} unique skills`);
  } catch (err) {
...
async function ensureDataLoaded() {
  if (isDataLoaded && allMembers.length > 0) {
    return true;
  }
  ...
  return isDataLoaded && allMembers.length > 0;
}
...
async function searchBySkills() {
  ...
  if (!dataReady) {
    container.innerHTML = '<p style="color:#f66; text-align:center;">Failed to load data. Please refresh the page.</p>';
    return;
  }
  ...
  console.log('🔍 Searching for skills:', searchSkills);
  console.log('📊 Searching through', allMembers.length, 'members');
  ...
  // Sort by relevance (more matching skills first)
  results.sort((a, b) => {
...
async function createProfileCard(member, currentUserCommunityId) {
  ...
  let connStatus = { status: 'none', canConnect: true };
  let showEmail = false;
  
  try {
    connStatus = await getConnectionStatus(member.id);
    showEmail = await canSeeEmail(member.id);
  } catch (err) {
    console.warn('Could not get connection status:', err);
  }
